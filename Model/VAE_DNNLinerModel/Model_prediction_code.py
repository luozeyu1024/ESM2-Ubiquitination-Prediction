...
class ContinuousResidualVAE(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=1280, z_dim=100, loss_type='RMSE', reduction='sum'):
        ...
        # Attributes for loss type and reduction
    ...
    def decode(self, z):
        ...
        h = self.resblock3(h)
        h = self.resblock4(h)
        return self.fc4(h)  # Output layer without activation
    ...
    def get_model_inference_z(self, x, seed=None):
        """
        Return the latent vector z for the input x.
...
def validate(model, device, val_loader, criterion):
    ...
    with torch.no_grad():  # Disable gradient computation
        ...
        for features, labels in val_loader:
            ...
            # Adjust labels to match the shape of outputs
...
# Load DNN model
best_params = pd.read_excel(open_path + r"\ESM2-Ubiquitination-Prediction\Model\VAE_DNNLinerModel\VAE_DNNLinearModel_best_hyperparameters.xlsx")
best_params = dict(zip(best_params.iloc[:,0], best_params.iloc[:,1]))
use_pos_weight = bool(best_params['use_pos_weight'])
vae_100_dnnliner_model = DNNLinearModel(input_dim = 100)
# Combined model
vae_dnnliner_model = CombinedModel(vae_model, vae_100_dnnliner_model)
# Load the combined model
vae_dnnliner_model.load_state_dict(torch.load(open_path + r"\ESM2-Ubiquitination-Prediction\Model\VAE_DNNLinerModel\VAE_DNNLinerModel.pth"))
vae_dnnliner_model.to(device)
...
# Perform inference
test_loss, test_mcc = validate(vae_dnnliner_model, device, test_loader, criterion)
